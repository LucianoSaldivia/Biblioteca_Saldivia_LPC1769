...
#define U1_BORRAR_FLAG_RX
#define U1_BORRAR_FLAG_TX

#define U1_REGISTRO_RX		U1RBR
#define U1_REGISTRO_TX		U1THR
#define U1_LSR				U1LSR
...
volatile uint8_t U1_Buf_RX[U1_MAX_RX];
volatile uint8_t U1_Buf_TX[U1_MAX_TX];

volatile uint8_t U1_Index_In_Rx = 0;
volatile uint8_t U1_Index_Out_Rx = 0 ;

volatile uint8_t U1_Index_In_Tx = 0;
volatile uint8_t U1_Index_Out_Tx = 0 ;

volatile uint8_t U1_TxEnCurso;   // Flag de TX en curso
...
void UART1_Init ( void ){
    //1.- Registro PCONP - bit 3 en 1 energiza la UART1
    PCONP |= ( 0x01 << 4 );

    //2.- Registro PCLKSEL0 - bits 6 y 7 en 0 seleccionan que el clk de la UART1 sea 25MHz
    PCLKSEL0 &= ~( 0x03 << 8 );

	//3.- Set-up: Largo de trama,Paridad, Stop, etc.
    U1LCR |= ( 0x03 << 0 );         // Bit 0:1. Largo de trama. 11 es 8 bits
    U1LCR &= ~( 0x01 << 2 );		// Bit 2. Stop bits. 0 es 1 bit
    U1LCR &= ~( 0x01 << 3 );		// Bit 3. Parity enable. 0 es desactivada
    U1LCR &= ~( 0x01 << 6 );		// Bit 6. Break Control. 0 es disabled
	...
	*/
    ...
    // DLL y DLM
    U1LCR |= ( 0x01 << 7 );         // Bit 7. DLAB == 1 para acceder a registros DLM y DLL

    U1DLM = 0;                  // DLM = 0
    U1DLL = 12;                 // DLL = 12

    U1FDR = 0;
    U1FDR |= ( 0x0E << 4 );     // Bits 4:7 . MULVAL = 14
    U1FDR |= 0x05;              // Bits 0:3 . DIVADDVAL = 5
    ...
    //7. Habilito las interrupciones de RX y TX en la UART1 (Registro U1IER)
    U1IER = 0x03;

    //8. Habilito la interrupción de la UART1 en el NVIC (Registro ISER0)
    ISER0 |= ( 0x01 << 6 );
}

void UART1_Inicializar( uint8_t PCLK, uint8_t BaudRate, uint8_t Data_Bits, uint8_t Parity, uint8_t Stop_Bits ){
    //1.- Registro PCONP - bit 3 en 1 habilita la UART1
    PCONP |= ( 0x01 << 4 );

    //2.- Registro PCLKSEL0 - bits 6 y 7 seleccionan el PCLK de la UART1
    PCLKSEL0 &= ~( 0x03 << 8 ); // Bits 6 y 7 en 0, para "reiniciar" el registro
    switch( PCLK ){
    ...
    U1LCR &= ~( 0x01 << 6 );	// Bit 6. Break Control. <0> es disabled (por default)
                                                      // <1> es enabled
    ...
    */
        ...
        */
            switch( PCLK ){
                case PCLK_CCLK_1:
                    ...
                    U1FDR = 0;                  // Pongo en 0 para después hacer U1FDR |=
                    U1FDR |= ( 0x00 << 0 );     // DivAddVal = 0
                    U1FDR |= ( 0x01 << 4 );     // MulVal = 1
                    break;
                ...
                case PCLK_CCLK_8:
                    ...
                    U1FDR = 0;                  // Pongo en 0 para después hacer U1FDR |=
                    U1FDR |= ( 0x01 << 0 );     // DivAddVal = 1
                    U1FDR |= ( 0x04 << 4 );     // MulVal = 4
                    break;
        ...
        */
            ...
            switch( PCLK ){
                ...
                case PCLK_CCLK_8:
                    ...
                    U1FDR = 0;                  // Pongo en 0 para después hacer U1FDR |=
                    U1FDR |= ( 0x02 << 0 );     // DivAddVal = 2
                    U1FDR |= ( 0x05 << 4 );     // MulVal = 5
                    break;
        ...
        */
            ...
            switch( PCLK ){
                ...
                case PCLK_CCLK_8:
                    ...
                    U1FDR = 0;                  // Pongo en 0 para después hacer U1FDR |=
                    U1FDR |= ( 0x00 << 0 );     // DivAddVal = 1
                    U1FDR |= ( 0x0B << 4 );     // MulVal = 11
                    break;
        ...
        */
            ...
            switch( PCLK ){
                ...
                case PCLK_CCLK_8:
                    ...
                    U1FDR = 0;                  // Pongo en 0 para después hacer U1FDR |=
                    U1FDR |= ( 0x03 << 0 );     // DivAddVal = 3
                    U1FDR |= ( 0x08 << 4 );     // MulVal = 8
                    break;
        ...
        */
            ...
            switch( PCLK ){
                ...
                case PCLK_CCLK_8:
                    ...
                    U1FDR = 0;                  // Pongo en 0 para después hacer U1FDR |=
                    U1FDR |= ( 0x05 << 0 );     // DivAddVal = 5
                    U1FDR |= ( 0x0D << 4 );     // MulVal = 13
                    break;
        ...
        */
            ...
            switch( PCLK ){
                case PCLK_CCLK_1:
                    ...
                    U1FDR = 0;                  // Pongo en 0 para después hacer U1FDR |=
                    U1FDR |= ( 0x07 << 0 );     // DivAddVal = 7
                    U1FDR |= ( 0x0C << 4 );     // MulVal = 12
                    break;
    ...
    //7. Habilito las interrupciones de RX y TX en la UART1 (Registro U1IER)
    U1IER = ( 0x00 << 0 );   // Sin Interrupciones
    U1IER |= ( 0x01 << 0 );  // Habilito interrupciones RX
    U1IER |= ( 0x02 << 0 );  // Habilito interrupciones TX
...
void UART1_IRQHandler ( void ){
    ...
    do {
        ...
        if ( Interrupcion == RX ) {
            ...
            UART1_PushRx();	// Guardo el dato recibido en el BufferRx
        }
...
void UART1_StartTx( void ){
    if( U1_Index_In_Tx != U1_Index_Out_Tx ){
        U1_REGISTRO_TX = dato;	// Fuerzo la transmisión del primer dato
    }
}

void UART1_PopTx( void ){
    if( U1_Index_In_Tx != U1_Index_Out_Tx ){                // Si hay dato para transmitir
        U1_REGISTRO_TX = U1_Buf_TX[ U1_Index_Out_Tx ];    // Transmito el dato
        U1_Index_Out_Tx ++;                             // Muevo el índice de salida de Tx
        U1_Index_Out_Tx %= U1_MAX_TX;
    }
    else{                                           // Si no hay más datos a enviar, terminó
        U1_TxEnCurso = 0;                           // la transmisión, entonces limpio el flag
    }
}

void UART1_PushRx( void ){
    U1_Buf_RX[ U1_Index_In_Rx ] = U1_REGISTRO_RX;
    U1_Index_In_Rx ++;
    U1_Index_In_Rx %= U1_MAX_RX;
}
